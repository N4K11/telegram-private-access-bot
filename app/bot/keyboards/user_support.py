...
def _help_button_specs() -> list[tuple[str, str]]:
    buttons: list[tuple[str, str]] = []
    seen_template_keys: set[str] = set()

    for entry in all_legal_text_entries():
        buttons.append((entry.button_text, f'menu:user:legal:{entry.slug}'))
        seen_template_keys.add(entry.template_key)

    for entry in all_content_entries():
        if entry.template_key in seen_template_keys:
            continue
        buttons.append((entry.button_text, f'menu:user:content:{entry.slug}'))
    return buttons
...
def user_support_overview_keyboard(*, open_ticket_id: int | None) -> InlineKeyboardMarkup:
    ...
    for text, callback_data in _help_button_specs():
