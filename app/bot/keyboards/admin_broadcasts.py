...
from app.services.broadcasts import BroadcastCampaignSnapshot
...
def admin_broadcast_filter_keyboard() -> InlineKeyboardMarkup:
    ...
    for label, filter_name in BASE_BROADCAST_FILTERS:
        builder.button(
            text=label,
            callback_data=f"menu:admin:broadcasts:filter:{filter_name}",
        )
    builder.button(
        text="По тарифу",
        callback_data="menu:admin:broadcasts:pick-filter:tariff",
    )
    builder.button(
        text="По каналу",
        callback_data="menu:admin:broadcasts:pick-filter:channel",
    )
